        ...
        "detections": [
            {
                ...
                "category": "karakter",
                "status": "gained",
                "evidence": "Student menyatakan ingin menjadi Hafiz agar bisa bermanfaat bagi umat."
            },
            {
                "parameter_name": "Prinsip belajar",
                "category": "mental",
                "status": "gained",
                "evidence": "Student belajar dengan tekun di kelas dan mencoba memahami materi secara mendalam."
            },
            {
                "parameter_name": "Keinginan untuk selalu mengembangkan diri.",
                "category": "softskill",
                "status": "gained",
                "evidence": "Student bertanya tentang materi tambahan di luar jam pelajaran."
            }
        ],
        "insight": "Student menunjukkan motivasi belajar yang tinggi dan kesadaran akan tujuan hidupnya. Perkembangan kognitif dan emotionalnya sangat stabil.",
...
#   - transcript: isi laporan student (konteks kondisi student)
...
def generate_treatment(transcript: str, unachieved_per_cat: dict) -> dict:
    ...
    tone = "Pertahankan momentum positif student." if is_positive else "Berikan dukungan ekstra karena student sedang menghadapi hambatan."
    ...
    for cat, items in unachieved_per_cat.items():
        ...
        for item in items:
            ...
            if cat == "karakter":
                action = f"Gunakan pendekatan personal untuk menguatkan aspek '{item['main_name']}'. {tone} Ajarkan student cara {item['detail_text'].lower()} melalui keteladanan harian di asrama."
            elif cat == "mental":
                action = f"Fokus pada penguatan spiritualitas terkait '{item['main_name']}'. Musyrif disarankan mengajak student melakukan {item['detail_text'].lower()} secara rutin sebagai bagian dari riyadhah."
            else: # softskill
                action = f"Berikan ruang bagi student untuk mempraktikkan '{item['main_name']}'. Tantang student untuk menunjukkan kemampuan {item['detail_text'].lower()} dalam kegiatan kelompok minggu ini."
